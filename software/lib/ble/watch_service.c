...
void ble_watch_send_reply_packet(uint8_t * reply_ptr, uint8_t len)
{
    uint32_t error_code;
    ...
    if (ble_watch_object.conn_handle != BLE_CONN_HANDLE_INVALID) {
        ...
        error_code = sd_ble_gatts_hvx(ble_watch_object.conn_handle, &hvx_params);
        APP_ERROR_CHECK(error_code);
    }
}
...
/**
 * Handle a GATT service write event. This will call the request_handler when
 * the request characteristic is written.
 */
static void on_write(ble_evt_t * evt_ptr)
{
    ble_gatts_evt_write_t * evt_write_ptr;
    evt_write_ptr = &evt_ptr->evt.gatts_evt.params.write;
 ...
 * value_buf -- buffer for the characteristic value.                    
 ...
 */
static void char_add(uint16_t uuid, uint16_t value_len, uint8_t * value_buf,
...
void ble_watch_init(ble_watch_request_handler_t handler)
{
    uint32_t error_code;
    ...
    ble_uuid128_t base_uuid = ble_watch_BASE_UUID;
    service_uuid.uuid = ble_watch_UUID_SERVICE;
    ...
    ble_watch_object.request_handler = handler; // FIXME is this correct syntax? It works...
}
